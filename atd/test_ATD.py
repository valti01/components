...
from data.open_set import get_out_testing_datasets, TensorAndCompression, CompressedDataset, split_data
...
def old_algorithm(global_min_distance, componentnumber, targetlabel, randomseed, distance_norm, original_images):
    ...
    compressed = CompressedDataset(root='./data', train=False, download=True, transform=transform)
    compressed = split_data(compressed, size=1000)
    after_time = time.time()
    print(f"svd completed in {after_time-current_time}, beginning search")
    original_images.reshape(1000, 3, 32, 32)
    while componentnumber < 3072:
        l = [x for (x, y) in compressed]
        compressed_images = torch.cat(l, 0)
        compressed_images = compressed_images.detach().numpy()
        compressed_images = compressed_images.reshape(1000, 3, 32, 32)
        compressed_images = np.array(compressed_images)
        if distance_norm == "Linf":
            distance = np.max(np.abs(original_images - compressed_images), axis=(1, 2, 3))  # linf
        if distance_norm == "L2":
            distance = np.sqrt(np.sum((original_images - compressed_images) ** 2, axis=(1, 2, 3)))  # l2
...
if __name__ == '__main__':
    ...
    print(x_test.shape)
    compnumber = args.comps
    if args.global_min_distance is not None:
        compnumber = old_algorithm(global_min_distance=args.global_min_distance, componentnumber=args.comps, targetlabel=args.targetlabel, randomseed=0, distance_norm=args.distance_norm, original_images=x_test)


    # out datasets
    out_names, out_datasets = get_out_testing_datasets(out_names, comps=compnumber, targetlabel=args.targetlabel,
                                                       mindistance=args.adaptive_min_distance)
    ...
    for i, score_out_dataset in enumerate(scores_out):
        ...
        distances = np.max(np.abs(x_test - x_test_ood), axis=(1, 2, 3))
        print(clean, inattack, outattack, bothattack)
        auc_data = []
        auc_data.append({
            "adaptive_min_distance": args.adaptive_min_distance,
            "Dataset": args.out_datasets,
            "min_distance": np.min(distances),
            "max_distance": np.max(distances),
            "average_dsitance": np.mean(distances),
            "clean_auc": clean,
            "in_auc": inattack,
            "out_auc": outattack,
            "a-auc": bothattack,
            "targetlabel": args.targetlabel,
            "global_min_distance":args.global_min_distance,
        ...
        if os.path.exists(file_path):
            ...
            with pd.ExcelWriter(file_path, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
                auc_df.to_excel(writer, sheet_name='Sheet1', index=False, header=False,
                                startrow=writer.sheets['Sheet1'].max_row)
        else:
            # If the file doesn't exist, create a new Excel file with the data
            auc_df.to_excel(file_path, index=False, sheet_name='Sheet1')
