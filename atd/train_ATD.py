    ...
    for epoch in range(num_epochs):
        ...
        for i, ((x, y), (x_out, y_out)) in enumerate(zip(trainloader, trainloader_out), 0):
            ...
            if training_type == 'adv':
                delta_x = attack_pgd(forward_pass, x, torch.ones_like(y, dtype=torch.float32).to(device), epsilon=eps,
                                     alpha=alp, attack_iters=attack_iters)

            delta_x_out = torch.zeros_like(x_out)
            if training_type == 'adv':
                delta_x_out = attack_pgd(forward_pass, x_out, torch.zeros_like(y_out, dtype=torch.float32).to(device),
                                         epsilon=eps, alpha=alp, attack_iters=attack_iters)
            ...
            output_real, output_fake, output_out = output_real_fake_out[:b_size], output_real_fake_out[
                                                                                  b_size:2 * b_size], output_real_fake_out[
                                                                                                      2 * b_size:]
